...
for i in range(0, NGames):
    ...
    # 남아 있는 문을 결정한다.
    OtherDoor=list(set(["Door1","Door2", "Door3"])-set([UserDoor, CarDoor]))[0]
    
    # 사용자가 선택을 바꾸지 않아서 성공하면 Ture를 리스트에 추갛나다. True는 정수 1이다.
    UnchangeGame.append(UserDoor==CarDoor)
    
    # 사용자가 선택을 ㄹ바꾸어서 성공하면 True를 리스트에 추가한다. True는 정수 1이다.
